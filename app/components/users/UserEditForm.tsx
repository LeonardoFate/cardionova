...
const editUserSchema = z.object({
  ...
  role: z.enum(["doctor", "secretary", "admin"]),
  phone: z.string()
    .optional()
    .refine(
...
interface UserEditFormProps {
  user: UserToEdit;
  onSubmit: (values: EditUserFormValues, certifications: string[]) => Promise<void>;
  onCancel: () => void;
}
...
export function UserEditForm({ user, onSubmit, onCancel }: UserEditFormProps) {
  ...
  useEffect(() => {
    const certList = user.certifications
      ? user.certifications.split(", ").filter(c => c.trim() !== "")
      : [""];
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input type="password" {...field} placeholder="Dejar vacío para no cambiar" />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
